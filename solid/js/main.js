...
class Quiz {
  constructor({ slider, ableOnSelect, onSubmit, onChangeSlide }) {
    ...
    this.inputs = this.el.querySelectorAll(
      'label.quiz-item input[type="radio"], label.quiz-item input[type="checkbox"]'
    );
    this.inputsText = this.el.querySelectorAll('label.quiz-item input[type="text"]');
    this.pageAllowed = 0;
    this.data = {};
  ...
  setupItems() {
    ...
    this.inputsText.forEach(input => {
      input.addEventListener('input', e => {
        if (!e.target.hasAttribute('data-missable')) {
          if (e.target.value !== '') {
            this.enableNext();
          } else {
            this.disableNext();
        ...
        const page = input.closest('.quiz__page');
        const multiply = page.hasAttribute('data-multiply');
        const key = input.getAttribute('name');
        const value = input.value;
        value && this.setData(key, value, multiply);
      });
    });
  }
  getHiddenInputs() {
  ...
  setup() {
    ...
    quiz.actions.next.addEventListener('click', function (e) {
      quiz.slider.go('+');
      const idx = quiz.slider.index;

      quiz.pageAllowed === idx
        ? quiz.disableNext()
        : quiz.enableNext();
      quiz.onChangeSlide();
    });
    quiz.slider.on('move', function (next, prev) {
      const current = quiz.progress.pageIdx;
  ...
  setup() {
    ...
    this.triggers.forEach((el) => {
      el.addEventListener('click', (e) => {
        this.showingHandler();
        if (!this.opened) {
          this.onFirstOpen && this.onFirstOpen();
          this.opened = true;
        }
      });
    });
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const quiz = new Quiz({
    ableOnSelect: true,
    onChangeSlide() {
      const [curr, len] = [quiz.slider.index, quiz.slider.length];
      if (curr === len - 1) {
  ...
  const callForm = callModal.el.querySelector('form');
  callForm.addEventListener('submit', e => {
    sendForm(callForm);
  });
  const callMaskedElem = callForm.querySelector('input[type="tel"]');
  const callMask = IMask(callMaskedElem, maskOptions);
  callMaskedElem.addEventListener('paste', e => {
    let data = e.clipboardData.getData('text');
    if (data.slice(0, 1) == '8') {
      e.preventDefault();
      data = data.slice(1);
      callMask.unmaskedValue = data;
    }
  });

  const quizMaskedElem = quiz.form.querySelector('input[type="tel"]');

  const mask = IMask(quizMaskedElem, maskOptions);

  quizMaskedElem.addEventListener('paste', e => {
    let data = e.clipboardData.getData('text');
    if (data.slice(0, 1) == '8') {
      e.preventDefault();
      data = data.slice(1);
  ...
  const labeled = document.querySelectorAll('.labeled input');
  labeled.forEach((elem) => {
    elem.closest('.labeled').addEventListener('click', e => {
      const input = e.target.querySelector('input');
      input && input.focus();
    });
    elem.addEventListener('focus', e => {
      e.target.closest('.labeled').classList.add('focus');
    });
    elem.addEventListener('blur', e => {
      if (!e.target.value) {
        e.target.closest('.labeled').classList.remove('focus');
      }
    });
  });
});