...
var news = {
    ...
    // slides: document.querySelector('.news-grid'),
};
if (window.innerWidth < 600) {
    news.slide = document.querySelector('.news-grid');
}

if (window.innerWidth < 600 && window.innerWidth > 450) {  //TODO закончить это все
    var firstNewsItem = news.items[0];
    for (var i = 0; i < newsSlider.length; i++) {
        newsSlider.remove(i);
    }
    for (var i = 0; i < news.items.length; i+=2) {
        var elem1 = news.items[i];
        var elem2 = news.items[i+1];
        var html;
        if (elem2) {
            html = '<li class="splide__slide news__slide">' + elem1.outerHTML + elem2.outerHTML + '</li>';
        } else {
            html = '<li class="splide__slide news__slide">' + elem1.outerHTML + '</li>';
        }
        newsSlider.add(html, -1);
    }
    // newsSlider.options.gap = 20;
